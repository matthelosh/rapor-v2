...
const simpan = async () => {
    await router.post(route('dashboard.post.store'), 
            { data: post.value }, {
...
const onCoverChange = async (e) => {
    ...
    fd.append("image", file)
    await axios.post(
        route('dashboard.post.image.upload'), 
            fd)
            .then(res => {
...
const listFiles = async() => {
    axios.post(route('dashboard.post.image.list'))
            .then(res => {
        ...
        <el-scrollbar max-height="60vh">
            <div class="post-writer grid grid-cols-8 gap-4">
                <div class="writer col-span-6">
                    <el-input v-model="post.title" placeholder="Judul Tulisan" style="margin-bottom: 20px; font-size: 1.5em;  outline: none; height: 40px; font-weight: bold; color: #345676;border:none;"></el-input>
                    <element-tiptap v-model:content="post.content" :extensions="extensions" />
                </div>
                <div class="post-meta col-span-2 border p-2 bg-slate-50 shadow">
                    <el-divider>Properti Tulisan</el-divider>
                    <h3>Cover Tulisan</h3>
                    <input type="file" ref="fileCover" accept=".jpg, .jpeg, .JPG, .JPEG, .png, .PNG, .bmp, .BMP, .svg, .SVG" @change="onCoverChange" class="hidden" />
                    <div class="cover-box hover:cursor-pointer" @click="$refs.fileCover.click()" >
                        <el-image :src="post.cover" fit="scale-down" style="width: 100%; height: 150px;background: #efefef; border-radius: 5px; border: 1px solid #ddf; display: flex; justify-content: center; align-items: center;">
                            <template #error>
                                <Icon icon="mdi:image-edit" class="text-8xl text-sky-300" />
                            </template>
                        </el-image>
                    </div>
                    <h3 class="mt-4">Kategori Tulisan</h3>
                    <el-select v-model="post.category" placeholder="Kategori">
                        <el-option v-for="(kat, k) in ['Berita', 'Pengumuman', 'Tutorial']" :value="kat"></el-option>
                    </el-select>
                    <h3 class="mt-4">Tipe Tulisan</h3>
                    <el-select v-model="post.type" placeholder="Kategori">
                        <el-option v-for="(tipe, t) in ['post', 'page', ]" :value="tipe"></el-option>
                    </el-select>
                </div>
            </div>
        </el-scrollbar>
    ...
    <template #footer>
        ...
        <div class="file-list bg-slate-200 p-4" v-if="fileBrowser">
            <template v-for="(file, f) in files" :key="f">
                <el-image :src="`/storage/${file}`" style="margin: 10px; width: 100px; cursor: pointer;" width="200px">
                    <template #error>
                        {{ file }}
                    </template>
                </el-image>
