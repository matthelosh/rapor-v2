...
const getPekerjaan = async () => {
    axios.get(route("dashboard.siswa.ortu.pekerjaan.index")).then((res) => {
...
const simpan = async () => {
    ...
    axios
        .post(route("dashboard.siswa.ortu.store"), { ortu: ortu.value })
        .then((res) => {
            ElNotification({
                title: "Info",
...
onBeforeMount(async () => {
    ...
    if (props.siswa.ortus.length > 0) {
        ortu.value.ayah = props.siswa.ortus.filter(
            (ortu) => ortu.relasi == "Ayah",
        )[0];
        ortu.value.ibu = props.siswa.ortus.filter(
            (ortu) => ortu.relasi == "Ibu",
        )[0];
        ortu.value.wali = props.siswa.ortus.filter(
    ...
    <el-dialog v-model="props.open" :show-close="false">
        <template #header="{ close }">
            <div class="header flex items-center justify-between">
                <h3 class="uppercase">
                    Formulir Orang Tua / Wali dari {{ siswa.nama }}
                </h3>
