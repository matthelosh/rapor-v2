...
<el-dialog fullscreen v-model="show" :show-close="false">
    <template #header>
        <div class="flex items-center justify-between">
            <h3 class="font-bold text-lg text-sky-800">Cetak Jurnal SPN {{ props.jilid.nama }}</h3>
        ...
        <el-row :gutter="20">
            <el-col :span="18">
                <div class="cetak">
                    <el-card class="mb-4">
                        <Kop />
                        <h3 class="text-center font-black uppercase my-8 text-lg">Laporan Berkala Sekolah Plus Ngaji</h3>
                        <table class="mx-auto my-6">
                            ...
                            <tr>
                                <td>Nama Guru</td>
                                <td class="px-2">:</td>
                                <td>{{ page.props.auth.user.userable.nama }}</td>
                                <td class="px-4"></td>
                                <td>Semester</td>
                                <td class="px-2">:</td>
                            ...
                            <tr>
                                <td>NIP</td>
                                <td class="px-2">:</td>
                                <td>{{ page.props.auth.user.userable.nip }}</td>
                                <td class="px-4"></td>
                        ...
                        <div class="jurnals">
                            <el-alert v-if="jurnals.length < 1" type="error" style="width: 50%; margin: 20px auto; padding: 20px;" title="Pilih jurnal di samping" :show-close="false">
                            </el-alert>
                            <div class="items" v-else>
                                <template v-for="(jurnal,j) in jurnals">
                                    <el-card class=" mb-10 w-[80%] mx-auto group relative break-inside-avoid-page">
                                        <Icon icon="mdi:trash-can" class="hidden group-hover:block text-red-500 text-2xl hover:cursor-pointer absolute right-4" @click="removeJurnal(j)" />
                                        <table class="w-full border border-black">
                                            <tr class="bg-slate-100">
                                                <td class="p-2 border border-black" >
                                                    <h3 class="font-bold uppercase text-lg">Materi</h3>    
                                                </td>
                                                <td class="border border-black" align="center">
                                                    :  
                                                </td>
                                                <td class="p-2 border border-black"    >
                                                    <h3 class="font-bold uppercase text-lg">{{ jurnal.materi }}</h3>    
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="p-2 border border-black">
                                                ...
                                                <td class="p-2 border border-black">
                                                    <h3 contenteditable="true">{{ dayjs(jurnal.created_at).format('DD MMMM YYYY') }}</h3>    
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="p-2 border border-black" >
                                            ...
                                            <tr>
                                                <td class="text-center p-2 border border-black" colspan="3" align="center">
                        ...
                        <div class="ttd grid grid-cols-10">
                            <div class="col-span-4 ttd-ks text-center">
                                <p>Mengetahui,</p>
                                <p>Kepala {{ page.props.auth.user.userable.sekolahs[0].nama }}</p>
                                <p class="mt-10 font-bold underline"><span class="uppercase">{{ page.props.auth.user.userable.nama }}</span>, {{page.props.auth.user.userable.gelar_belakang}}</p>
                            ...
                            <div class="col-span-4 ttd-gpai text-center">
                                <p>Malang, {{ dayjs(new Date()).format('DD MMMM YYYY') }}</p>
            ...
            <el-col :span="6">
                <el-card>
                    ...
                    <h3 class="font-bold uppercase text-sky-700 mb-2">Data Jurnal {{ props.jilid.nama }}:  </h3>
                    <ul>
                        <li v-for="(jurnal,j) in savedJurnals" class="mb-2 flex items-center gap-2 border-b border-dashed hover:bg-sky-50 py-1">
                            <el-button circle size="small" type="primary" @click="addJurnal(j)">
                                <Icon icon="mdi:plus" />
                            </el-button> {{ dayjs(jurnal.created_at).format('DD/MM/YYYY') }} | {{ jurnal.materi }}
                        </li>
                    </ul>
                </el-card>
            </el-col>
