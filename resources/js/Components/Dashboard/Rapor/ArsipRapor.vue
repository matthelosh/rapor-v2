...
<template>
    <el-dialog v-model="props.open" fullscreen :show-close="false">
        <template #header>
            <div
                class="dialog-toolbar flex items-center justify-between border-b py-2"
            >
                <span class="title"
                    >Lihat Arsip {{ props.rombel.label }} Tahun Pelajaran
                    {{ props.tapel.label }} |
                    {{ props.rombel.wali_kelas?.nama }}</span
                >
