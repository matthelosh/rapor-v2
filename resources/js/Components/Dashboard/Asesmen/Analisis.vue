...
const skorPgk = (s) => {
    ...
    for (let i = 0; i < analisis.value[s].pgk.jawabans.length; i++) {
        ...
        skor += [...kunciSet].every((char) => jawab.has(char))
            ? 2
            : [...kunciSet].filter((char) => jawab.has(char)).length === 1
              ? 1
              : 0;
    }
...
const skorPs = (s) => {
    ...
    for (let i = 0; i < analisis.value[s].ps.jawabans.length; i++) {
        skor +=
            kunci.value.ps.kunci[i].toUpperCase() ==
            analisis.value[s].ps.jawabans[i].toUpperCase()
                ? 2
                : 0;
    }
...
const simpan = async () => {
    ...
    router.post(
        route("dashboard.analisis.store"),
        {
...
const showKunci = () => {
    ...
    ElMessageBox.alert(`${text}`, "Kunci Jawaban:", {
        type: "info",
        draggable: true,
        icon: false,
    });
...
<template>
    <div>
        ...
        >
            ...
            <div
                class="h-10 bg-slate-200 py-2 flex items-center justify-between"
            >
                <el-button-group size="small">
                    <el-button size="small" @click="unduhExcel"
                        >Export Excel</el-button
                    >
                    <el-button size="small" @click="cetak">Cetak</el-button>
                    <el-button size="small" @click="simpan"
                        >Simpan Analisis</el-button
                    >
                ...
                <el-button-group size="small">
                    <el-button
                        ...
                        type="primary"
                        @click="showKunci"
                        >Lihat Kunci</el-button
                    >
                </el-button-group>
            </div>
            <div
                class="max-w-screen overflow-x-auto max-h-[80vh] overflow-y-auto py-4"
            >
                <table>
                    <thead>
                        <tr class="bg-sky-600 text-white">
                            <th class="border py-1 px-2">No</th>
                            <th class="border py-1 px-2">Nama</th>
                            <template v-for="(p, i) in kunci.pg.kunci">
                                <th class="border-b p-1">{{ i + 1 }}</th>
                            </template>
                            <th class="border-b p-1">SUB PG</th>
                            <template v-for="(p, i) in kunci.pgk.kunci">
                                <th class="border-b p-1">{{ i + 1 }}</th>
                            </template>
                            <th class="border-b p-1">SUB PGK</th>
                            <template v-for="(p, i) in kunci.ps.kunci">
                                <th class="border-b p-1">{{ i + 1 }}</th>
                            </template>
                            <th class="border-b p-1">SUB PS</th>
                            <template v-for="(p, i) in kunci.is.kunci">
                                <th class="border-b p-1">{{ i + 1 }}</th>
                            </template>
                            <th class="border-b p-1">SUB Isian</th>
                            <template v-for="(p, i) in kunci.ur.kunci">
                                <th class="border py-1 px-2">{{ i + 1 }}</th>
                            </template>
                            <th class="border-b p-1">SUB Uraian</th>
                            <th class="border-b p-1">SKOR</th>
                        </tr>
                    </thead>
                    <tbody>
                        <template v-for="(siswa, s) in analisis">
                            <tr class="even:bg-slate-100">
                                ...
                                <td class="border py-1 px-2 w-[200px] text-sm">
                                    {{ siswa.nama }}
                                </td>
                                <template v-for="(pg, p) in kunci.pg.kunci">
                                    <td class="border py-1 px-2">
                                        <input
                                            :ref="(el) => (pgRefs[s][p] = el)"
                                            @input="
                                                (val) => validatePG(val, s, p)
                                            "
                                            v-model="
                                                analisis[s]['pg']['jawabans'][p]
                                            "
                                            maxlength="1"
                                            required
                                            class="w-6 text-center p-0 border-t-0 border-r-0 border-l-0 border-b outline-none active:outline-none focus:outline-none focus:bg-sky-50"
                                        />
                                    </td>
                                </template>
                                <td
                                    class="border py-1 px-2 text-center font-bold"
                                >
                                    {{ skorPg(s) }}
                                </td>
                                <template v-for="(pg, p) in kunci.pgk.kunci">
                                    <td class="border py-1 px-2">
                                        <input
                                            :ref="(el) => (pgkRefs[s][p] = el)"
                                            @change="
                                                (val) => validatePgk(val, s, p)
                                            "
                                            v-model="
                                                analisis[s]['pgk']['jawabans'][
                                                    p
                                                ]
                                            "
                                            maxlength="2"
                                            required
                                            class="w-8 text-center p-0 border-t-0 border-r-0 border-l-0 border-b outline-none active:outline-none focus:outline-none focus:bg-sky-50"
                                        />
                                    </td>
                                </template>
                                <td
                                ...
                                <template v-for="(ps, p) in kunci.ps.kunci">
                                    <td class="border py-1 px-2">
                                        <input
                                            :ref="(el) => (psRefs[s][p] = el)"
                                            @change="
                                                (val) => validatePS(val, s, p)
                                            "
                                            v-model="
                                                analisis[s]['ps']['jawabans'][p]
                                            "
                                            maxlength="2"
                                            required
                                            class="w-8 text-center p-0 border-t-0 border-r-0 border-l-0 border-b outline-none active:outline-none focus:outline-none focus:bg-sky-50"
                                ...
                                <template v-for="(is, p) in kunci.is.kunci">
                                    <td class="border py-1 px-2">
                                        <input
                                            v-model="
                                                analisis[s]['is']['jawabans'][p]
                                            "
                                            maxlength="2"
                                            required
                                            class="w-12 text-center p-0 border-t-0 border-r-0 border-l-0 border-b outline-none active:outline-none focus:outline-none focus:bg-sky-50"
                                            min="0"
                                            max="1"
                                            step="0.1"
                                            type="number"
