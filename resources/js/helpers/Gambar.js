...
export const avatar = (guru = null) => {
    if (guru == null) {
        return (['superadmin', 'admin', 'admin_tp', 'org'].includes(page.props.auth.roles[0]))? '/img/user_l.png' : (page.props.auth.user.userable.jk == 'Laki-laki' ? '/img/user_l.png' : (page.props.auth.user.userable.agama == 'Islam' ? '/img/user_p_is.png' : '/img/user_p.png'))
    } else {
        if (guru.foto == null) {
